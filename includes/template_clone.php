...
function clone_template_table_recursive($pdo, $tb, $newSectionId, $newParentGroupId, &$tableIdMap, &$swapLinks, &$countLinks) {
    $insT = $pdo->prepare(
        'INSERT INTO raid_template_tables (section_id, parent_group_id, title, header_color, default_column_width, row_label_width, sort_order, bg_color, kind, count_categories)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    );
    $insT->execute([$newSectionId, $newParentGroupId, $tb['title'], $tb['header_color'], $tb['default_column_width'], $tb['row_label_width'], $tb['sort_order'], $tb['bg_color'], $tb['kind'], $tb['count_categories']]);
