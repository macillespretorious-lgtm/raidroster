...
function clone_template_to_guild($pdo, $sourceTemplateId, $targetGuildId, $newName) {
    ...
    $ins = $pdo->prepare('INSERT INTO raid_templates (guild_id, name, description, default_start_time, default_duration_minutes, size) VALUES (?, ?, ?, ?, ?, ?)');
    $ins->execute([$targetGuildId, $newName, $src['description'], $src['default_start_time'], $src['default_duration_minutes'], $src['size']]);
...
function clone_template_table_recursive($pdo, $tb, $newSectionId, $newParentGroupId, &$tableIdMap, &$swapLinks) {
    ...
    $insT->execute([$newSectionId, $newParentGroupId, $tb['title'], $tb['header_color'], $tb['default_column_width'], $tb['row_label_width'], $tb['sort_order'], $tb['bg_color'], $tb['kind']]);
