...
function copy_table_recursive($pdo, $tb, $newSectionId, $newParentGroupId) {
    $insT = $pdo->prepare(
        'INSERT INTO raid_tables (section_id, parent_group_id, title, sort_order, header_color, bg_color, default_column_width, source_table_id, kind, swap_default, count_categories, mark_style, mark_enabled, note_enabled, note_text, is_primary)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    );
    $insT->execute([$newSectionId, $newParentGroupId, $tb['title'], $tb['sort_order'], $tb['header_color'], $tb['bg_color'], $tb['default_column_width'], $tb['id'], $tb['kind'] ?? 'standard', $tb['swap_default'] ?? 0, $tb['count_categories'] ?? null, $tb['mark_style'] ?? 'star', $tb['mark_enabled'] ?? 0, $tb['note_enabled'] ?? 0, $tb['note_text'] ?? null, $tb['is_primary'] ?? 0]);
    ...
    $stmtCells = $pdo->prepare('SELECT row_id, column_id, text_content, bg_color, text_color, bold, font, icon, kind_override, text_align FROM raid_template_cells WHERE table_id = ?');
    ...
    $insCell = $pdo->prepare('INSERT INTO raid_cells (table_id, row_id, column_id, text_content, bg_color, text_color, bold, font, icon, kind_override, text_align) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    foreach ($rows as $r) {
        if ($r['kind'] === 'spacer') continue;
        foreach ($columns as $c) {
            if ($c['kind'] === 'spacer') continue;
            $tc = $tplCells[$r['srcId'] . '_' . $c['srcId']] ?? null;
            $insCell->execute([$newTableId, $r['id'], $c['id'], $tc['text_content'] ?? null, $tc['bg_color'] ?? null, $tc['text_color'] ?? null, $tc['bold'] ?? 0, $tc['font'] ?? null, $tc['icon'] ?? null, $tc['kind_override'] ?? null, $tc['text_align'] ?? null]);
        }
    }
