...
function copy_table_recursive($pdo, $tb, $newSectionId, $newParentGroupId) {
    ...
    $stmtCells = $pdo->prepare('SELECT row_id, column_id, text_content, bg_color, text_color, kind_override FROM raid_template_cells WHERE table_id = ?');
    ...
    $insCell = $pdo->prepare('INSERT INTO raid_cells (table_id, row_id, column_id, text_content, bg_color, text_color, kind_override) VALUES (?, ?, ?, ?, ?, ?, ?)');
    foreach ($rows as $r) {
        if ($r['kind'] === 'spacer') continue;
        foreach ($columns as $c) {
            if ($c['kind'] === 'spacer') continue;
            $tc = $tplCells[$r['srcId'] . '_' . $c['srcId']] ?? null;
            $insCell->execute([$newTableId, $r['id'], $c['id'], $tc['text_content'] ?? null, $tc['bg_color'] ?? null, $tc['text_color'] ?? null, $tc['kind_override'] ?? null]);
        }
    }
