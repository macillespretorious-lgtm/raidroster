...
function sync_table($pdo, $raidTableId, $tplTableId, &$diff, $apply, $confirmRemovals) {
    ...
    if ($apply) {
        ...
        $stmt->execute([$tplTableId]);
        $updCell = $pdo->prepare('UPDATE raid_cells SET text_content = ?, bg_color = ?, text_color = ?, kind_override = ? WHERE table_id = ? AND row_id = ? AND column_id = ?');
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $tc) {
            ...
            $updCell->execute([$tc['text_content'], $tc['bg_color'], $tc['text_color'], $tc['kind_override'], $raidTableId, $rId, $cId]);
        }
