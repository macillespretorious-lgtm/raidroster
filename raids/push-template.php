...
function sync_table($pdo, $raidTableId, $tplTableId, &$diff, $apply, $confirmRemovals) {
    ...
    $changes = diff_scalar_fields($raidTb, $tplTb, ['title', 'header_color', 'bg_color', 'default_column_width', 'sort_order', 'count_categories']);
    if ($changes) {
        $diff['tables']['changed'][] = ['id' => $raidTableId, 'label' => $raidTb['title'] ?: '(untitled table)', 'changes' => array_keys($changes)];
        if ($apply) {
            $pdo->prepare('UPDATE raid_tables SET title = ?, header_color = ?, bg_color = ?, default_column_width = ?, sort_order = ?, count_categories = ? WHERE id = ?')
                ->execute([$tplTb['title'], $tplTb['header_color'], $tplTb['bg_color'], $tplTb['default_column_width'], $tplTb['sort_order'], $tplTb['count_categories'], $raidTableId]);
        }
    }
